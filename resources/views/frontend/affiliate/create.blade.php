...
@section('seo_title', 'Daftar Affiliate - Sora Hotel Merauke')
@section('meta_description', 'Bergabunglah dengan program affiliate Sora Hotel Merauke dan dapatkan komisi menarik dari setiap pemesanan yang Anda referensikan.')
...
@section('content')
    <div class="min-h-screen bg-gray-900 flex items-center justify-center py-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        ...
        <div class="absolute top-0 left-0 w-full h-full overflow-hidden z-0">
            <div class="absolute top-0 right-0 -mr-20 -mt-20 w-96 h-96 bg-brand-gold rounded-full opacity-10 blur-3xl"></div>
            <div class="absolute bottom-0 left-0 -ml-20 -mb-20 w-96 h-96 bg-admin-secondary rounded-full opacity-10 blur-3xl"></div>
        ...
        <div class="relative z-10 w-full max-w-5xl">
            ...
            <div class="text-center mb-10">
                <span class="text-brand-gold font-bold uppercase tracking-[0.2em] text-xs mb-2 block">Partnership Program</span>
                ...
                <p class="text-gray-400 max-w-2xl mx-auto text-sm">
                    Dapatkan penghasilan tambahan dengan merekomendasikan Sora Hotel Merauke.
                </p>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-800 flex flex-col lg:flex-row">
                ...
                <div class="w-full lg:w-5/12 bg-gray-50 p-8 border-b lg:border-b-0 lg:border-r border-gray-100 flex flex-col justify-center">
                    ...
                    <ul class="space-y-6">
                        <li class="flex items-start gap-4">
                            <div class="w-12 h-12 rounded-full bg-yellow-100 flex items-center justify-center text-brand-gold flex-shrink-0 shadow-sm">
                        ...
                        <li class="flex items-start gap-4">
                            <div class="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center text-admin-secondary flex-shrink-0 shadow-sm">
                    ...
                    <div class="mt-8 p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
                        <p class="text-xs text-gray-600 italic text-center">
                            "Bergabung dengan Sora Hotel Affiliate sangat mudah dan menguntungkan."
                        </p>
                    </div>
                </div>
                ...
                <div class="w-full lg:w-7/12 p-8 lg:p-10 bg-white">
                    ...
                    @if ($errors->any())
                        <div class="mb-6 p-3 bg-red-50 border-l-4 border-brand-red text-red-700 rounded text-xs">
                    ...
                    <form method="POST" action="{{ route('affiliate.register.store') }}" class="space-y-4">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="name" value="{{ old('name', Auth::user()->name ?? '') }}" required 
                                           class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-gold focus:border-brand-gold bg-gray-50 py-2.5 {{ Auth::check() ? 'text-gray-500 cursor-not-allowed' : '' }}" 
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="password" name="password" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-gold focus:border-brand-gold bg-gray-50 py-2.5">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Konfirmasi Password</label>
                                    <input type="password" name="password_confirmation" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-gold focus:border-brand-gold bg-gray-50 py-2.5">
                                </div>
                            </div>
                            @endguest
                        </div>
                        ...
                        <div class="space-y-4 pt-2">
                            <div class="flex items-center gap-2 pb-2 border-b border-gray-100">
                                <span class="flex items-center justify-center w-6 h-6 rounded-full bg-brand-gold text-white text-xs font-bold">2</span>
                            ...
                            <div>
                                ...
                                <input type="tel" name="phone" value="{{ old('phone', Auth::user()->phone ?? '') }}" required placeholder="08xxxxxxxxxx" class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-gold focus:border-brand-gold bg-gray-50 py-2.5">
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div class="md:col-span-1">
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Bank</label>
                                    <select name="bank_name" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-gold focus:border-brand-gold bg-gray-50 py-2.5">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <input type="text" name="bank_account_number" value="{{ old('bank_account_number') }}" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-gold focus:border-brand-gold bg-gray-50 py-2.5" placeholder="Angka Saja">
                                </div>
                            </div>

                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Atas Nama (Sesuai Buku Tabungan)</label>
                                <input type="text" name="bank_account_holder" value="{{ old('bank_account_holder') }}" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-brand-gold focus:border-brand-gold bg-gray-50 py-2.5" placeholder="Nama Pemilik Rekening">
                            </div>
                        </div>

                        <div class="pt-6">
                            <button type="submit" class="w-full flex justify-center py-3.5 px-4 border border-transparent rounded-xl shadow-lg text-sm font-bold text-gray-900 bg-brand-gold hover:bg-brand-gold focus:outline-none transition-all transform hover:-translate-y-0.5">
                                Daftar Sekarang
                            </button>
                        </div>
                        ...
                        @guest
                            <p class="text-center text-xs text-gray-500 pt-2">
                                Sudah punya akun? <a href="{{ route('login') }}" class="font-bold text-brand-gold hover:underline">Masuk di sini</a>
                            </p>
                        @endguest
                    </form>
                </div>
