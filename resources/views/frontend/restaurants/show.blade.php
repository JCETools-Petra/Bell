...
@section('seo_title', $restaurant->seo_title ?: $restaurant->name . ' - Dining at Sora Hotel')
...
@section('content')
    ...
    <div class="relative bg-gray-900 pt-24 pb-16 sm:pb-20 overflow-hidden">
        ...
        <div class="container mx-auto px-4 relative z-10 text-center">
            <span class="inline-block py-1 px-3 rounded-full bg-brand-gold/20 text-brand-gold border border-brand-gold/30 text-xs font-bold tracking-widest uppercase mb-4 backdrop-blur-sm">
            ...
            <div class="flex items-center justify-center gap-6 text-gray-300 text-sm md:text-base">
                ...
                <span class="flex items-center gap-2"><i class="far fa-clock text-brand-gold"></i> {{ $restaurant->opening_hours }}</span>
                @endif
                @if($restaurant->location)
                <span class="flex items-center gap-2"><i class="fas fa-map-marker-alt text-brand-gold"></i> {{ $restaurant->location }}</span>
                @endif
            </div>
        </div>
    </div>
                ...
                <div class="lg:col-span-2 space-y-8">
                            ...
                            <div class="grid grid-cols-4 gap-2">
                                @foreach($restaurant->images as $image)
                                    <button onclick="changeImage('{{ asset('storage/' . $image->path) }}')" 
                                            class="relative h-20 md:h-24 rounded-lg overflow-hidden cursor-pointer opacity-70 hover:opacity-100 transition-all hover:ring-2 hover:ring-brand-gold">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm p-6 md:p-8 border border-gray-100">
                        <h3 class="text-2xl font-heading font-bold text-gray-900 mb-4 border-b border-gray-100 pb-4">
                            Tentang <span class="text-brand-gold">{{ $restaurant->name }}</span>
                        </h3>
                    ...
                    <div class="bg-gray-900 rounded-2xl shadow-lg p-8 text-white relative overflow-hidden">
                        <div class="absolute top-0 right-0 w-32 h-32 bg-brand-gold rounded-full filter blur-[60px] opacity-20"></div>
                        <h3 class="text-xl font-bold text-brand-gold mb-6 relative z-10">Signature Dishes</h3>
                            ...
                            <div class="p-6">
                                ...
                                <ul class="space-y-5">
                                    <li class="flex items-start gap-4">
                                        <div class="w-10 h-10 rounded-full bg-yellow-50 text-brand-gold flex items-center justify-center flex-shrink-0">
                                    ...
                                    <li class="flex items-start gap-4">
                                        <div class="w-10 h-10 rounded-full bg-blue-50 text-admin-secondary flex items-center justify-center flex-shrink-0">
                                    ...
                                    <li class="flex items-start gap-4">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-gray-900 font-medium">{{ $restaurant->location ?? 'Lobby Level, Sora Hotel' }}</span>
                                        </div>
                                    </li>
                                </ul>
                                ...
                                @else
                                    <a href="{{ route('contact.index') }}" 
                                       class="w-full flex items-center justify-center gap-2 bg-gray-900 hover:bg-brand-gold text-white hover:text-gray-900 font-bold py-3 rounded-lg transition-all">
                                        Hubungi Kami
                                    </a>
                                @endif
