...
<div class="page-wrapper">
  ...
  <section class="mice-info-section">
    <div class="mice-container">
      ...
      {{-- SPECIFICATIONS GRID --}}
      @if($mice->dimension || $mice->size_sqm)
  ...
  @php
    ...
    $hasCapacities = collect($layouts)->pluck('capacity')->filter()->isNotEmpty();
  @endphp

  @if($hasCapacities)
    <section class="capacity-section">
      <div class="mice-container">
        <h2 class="section-title">Layout Capacity</h2>
            ...
            @if($details['capacity'])
              <div class="capacity-card">
                @if(isset($settings[$details['icon_key']]))
                  <img src="{{ asset('storage/' . $settings[$details['icon_key']]) }}"
                       alt="{{ $name }} Layout"
                       class="capacity-icon">
                @endif
                <div class="capacity-name">{{ $name }}</div>
                <div class="capacity-value">{{ $details['capacity'] }} Pax</div>
              </div>
            @endif
          @endforeach
        </div>
