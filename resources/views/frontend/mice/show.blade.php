...
@section('seo_title', $mice->seo_title ?: $mice->name . ' - Sora Hotel Merauke')
    ...
    <div class="bg-gray-900 pt-24 pb-12 relative overflow-hidden">
        ...
        <div class="container mx-auto px-4 relative z-10">
            <nav class="flex text-sm text-gray-400 mb-4">
                ...
                <a href="{{ route('mice.index') }}" class="hover:text-brand-gold transition-colors">MICE</a>
                <span class="mx-2">/</span>
                <span class="text-white">{{ $mice->name }}</span>
            </nav>
            <h1 class="text-3xl md:text-5xl font-heading font-bold text-white mb-2">{{ $mice->name }}</h1>
            <p class="text-brand-gold text-lg font-medium flex items-center gap-2">
                            ...
                            <div class="grid grid-cols-4 gap-2">
                                @foreach($mice->images as $image)
                                    <button onclick="changeImage('{{ asset('storage/' . $image->path) }}')" 
                                            class="relative h-20 md:h-24 rounded-lg overflow-hidden cursor-pointer opacity-70 hover:opacity-100 transition-opacity focus:outline-none focus:ring-2 focus:ring-brand-gold">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            @foreach($features as $feature)
                                ...
                                <div class="flex items-start gap-3">
                                    <div class="w-8 h-8 rounded-full bg-blue-50 text-admin-secondary flex items-center justify-center flex-shrink-0">
                                ...
                                <form action="{{ route('mice.inquiries.store') }}" method="POST">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="text" name="customer_name" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-brand-gold focus:border-brand-gold" required placeholder="Nama Anda / Perusahaan">
                                        </div>

                                        <div>
                                            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Nomor WhatsApp</label>
                                            <input type="tel" name="customer_phone" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-brand-gold focus:border-brand-gold" required placeholder="08xxxxxxxxxx">
                                        </div>

                                        <div>
                                            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Jenis Acara</label>
                                            <select name="event_type" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-brand-gold focus:border-brand-gold">
                                        ...
                                        <div>
                                            ...
                                            <textarea name="event_other_description" rows="3" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-brand-gold focus:border-brand-gold" placeholder="Tanggal acara, jumlah peserta, atau request khusus..."></textarea>
                                        </div>
