...
@section('content')
@php
    // Perbaikan path Hero Image Setting
    $heroBgPath = $settings['hero_image_path'] ?? '';
    $heroBg = !empty($heroBgPath)
              ? (Str::contains($heroBgPath, 'storage/') ? asset($heroBgPath) : asset('storage/' . str_replace('public/', '', $heroBgPath)))
            ...
            <div class="carousel-inner">
                @foreach($heroSliders as $slider)
                    <div class="carousel-item {{ $loop->first ? 'active' : '' }}">
                        {{-- PERBAIKAN IMAGE HERO SLIDER --}}
                        <img src="{{ Str::contains($slider->image_path, 'storage/') ? asset($slider->image_path) : asset('storage/' . str_replace('public/', '', $slider->image_path)) }}" 
                             class="d-block w-100" 
            ...
            <div class="carousel-inner">
                @foreach($banners as $banner)
                    <div class="carousel-item {{ $loop->first ? 'active' : '' }}">
                        ...
                        {{-- PERBAIKAN IMAGE BANNER --}}
                        <img src="{{ Str::contains($banner->image_path, 'storage/') ? asset($banner->image_path) : asset('storage/' . str_replace('public/', '', $banner->image_path)) }}" 
                             class="d-block w-100" 
                             alt="Banner Image">
                            ...
                            <div class="modern-room-card h-100 position-relative overflow-hidden rounded-4 shadow-lg" style="transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);">
                                @if ($room->images->isNotEmpty())
                                    <div class="position-relative overflow-hidden" style="height: 320px;">
                                        {{-- PERBAIKAN IMAGE ROOMS --}}
                                        <img src="{{ Str::contains($room->images->first()->path, 'storage/') ? asset($room->images->first()->path) : asset('storage/' . str_replace('public/', '', $room->images->first()->path)) }}"
                            ...
                            <div class="modern-event-card position-relative overflow-hidden rounded-4" style="background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);">
                                @if ($mice->images->isNotEmpty())
                                    <div class="position-relative overflow-hidden" style="height: 280px;">
                                        {{-- PERBAIKAN IMAGE MICE --}}
                                        <img src="{{ Str::contains($mice->images->first()->path, 'storage/') ? asset($mice->images->first()->path) : asset('storage/' . str_replace('public/', '', $mice->images->first()->path)) }}"
                                             class="w-100 h-100 object-fit-cover"
                                             alt="{{ $mice->name }}"
                                             style="transition: transform 0.6s ease;"
                                             onerror="this.onerror=null;this.src='https://via.placeholder.com/600x400?text=No+Image';">
                            ...
                            <div class="modern-recreation-card position-relative overflow-hidden rounded-4 shadow-lg bg-white" style="transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);">
                                @if ($recreation->images->isNotEmpty())
                                    <div class="position-relative overflow-hidden" style="height: 300px;">
                                        @php
                                            // FIX: Ambil gambar pertama
                                            $imgObj = $recreation->images->first();
                                            // FIX: Cek apakah nama kolomnya 'image_path' (seperti Banner) atau 'path' (seperti Room)
                                            $imgPath = $imgObj->image_path ?? $imgObj->path ?? '';
                                        @endphp

                                        <img src="{{ Str::contains($imgPath, 'storage/') ? asset($imgPath) : asset('storage/' . str_replace('public/', '', $imgPath)) }}"
                                             class="w-100 h-100 object-fit-cover"
                                             alt="{{ $recreation->name }}"
                                             style="transition: transform 0.6s ease;"
                                             onerror="this.onerror=null;this.src='https://via.placeholder.com/600x400?text=No+Image';">
                                        ...
                                        <div class="position-absolute bottom-0 start-0 w-100 p-4 text-white">
                                            <h5 class="fw-bold mb-1 text-white" style="font-size: 1.5rem; text-shadow: 2px 2px 4px rgba(0,0,0,0.5); color: white !important;">{{ $recreation->name }}</h5>
                                        </div>
                                    </div>
                                @else
