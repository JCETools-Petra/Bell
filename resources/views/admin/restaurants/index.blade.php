<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.restaurants.create') }}" class="px-6 py-2.5 bg-brand-primary text-white font-bold rounded-xl shadow-lg shadow-brand-primary/30 hover:bg-brand-dark transition-all transform hover:-translate-y-0.5 flex items-center gap-2">
                <i class="fas fa-plus"></i> {{ __('Tambah Restoran Baru') }}
            </a>
        </div>
    </x-slot>
                            ...
                            <tbody class="bg-white divide-y divide-gray-50">
                                @forelse ($restaurants as $restaurant)
                                    <tr class="hover:bg-blue-50/30 transition-colors group">
                                        <td class="px-6 py-4 whitespace-nowrap font-medium text-brand-dark">{{ $restaurant->name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-600">{{ Str::limit($restaurant->description, 50) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if ($restaurant->images->isNotEmpty())
                                                <img src="{{ asset('storage/' . $restaurant->images->first()->path) }}" alt="{{ $restaurant->name }}" class="w-16 h-16 object-cover rounded-lg shadow-sm border border-gray-100">
                                            @else
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('admin.restaurants.edit', $restaurant) }}" class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all" title="Edit">
