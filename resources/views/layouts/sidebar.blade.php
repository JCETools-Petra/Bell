    ...
    <div class="flex-1 overflow-y-auto">
        <nav class="flex-1 space-y-1 py-4 px-2">
            ...
            @if(Auth::user()->role == 'admin')
                ...
                <a href="{{ route('admin.recreation-areas.index') }}" class="flex items-center rounded-lg mx-2 px-3 py-2.5 text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.recreation-areas.*') ? 'bg-gradient-to-r from-admin-primary to-admin-secondary text-white shadow-lg shadow-admin-primary/50' : 'hover:bg-admin-primary/10 hover:text-admin-primary hover:translate-x-1' }} border-l-2 {{ request()->routeIs('admin.recreation-areas.*') ? 'border-admin-accent' : 'border-transparent hover:border-admin-primary' }}">
                    <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 10l-2 1m0 0l-2-1m2 1v2.5M20 7l-2 1m2-1l-2-1m2 1v2.5M14 4l-2-1-2 1M4 7l2-1M4 7l2 1M4 7v2.5M12 21l-2-1m2 1l2-1m-2 1v-2.5M6 18l-2-1v-2.5M18 18l2-1v-2.5"/></svg>
                    Recreation Areas
                </a>
                <hr class="border-admin-primary/20 my-3 mx-4">
                ...
                <a href="{{ route('admin.activity-logs.index') }}" class="flex items-center rounded-lg mx-2 px-3 py-2.5 text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.activity-logs.*') ? 'bg-gradient-to-r from-admin-primary to-admin-secondary text-white shadow-lg shadow-admin-primary/50' : 'hover:bg-admin-primary/10 hover:text-admin-primary hover:translate-x-1' }} border-l-2 {{ request()->routeIs('admin.activity-logs.*') ? 'border-admin-accent' : 'border-transparent hover:border-admin-primary' }}">
                    <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    Activity Logs
                </a>
            ...
            @else
                {{-- Default untuk role lain (user, affiliate, dll) --}}
                <div class="px-4 py-8 text-center">
                    ...
                    <p class="text-sm text-gray-400">No admin access</p>
                    <p class="text-xs text-gray-500 mt-2">Role: {{ Auth::user()->role }}</p>
                </div>
            @endif
        </nav>
    </div>
