...
class MiceKitController extends Controller
{
    ...
    public function edit(MiceKit $miceKit)
    {
        return view('admin.mice_kits.edit', compact('miceKit'));
    }

    public function update(Request $request, MiceKit $miceKit)
    {
        $request->validate([
            'title' => 'required|string|max:255',
            'description' => 'required|string',
            ...
            'video_file' => 'nullable|mimetypes:video/mp4,video/mov,video/ogg,video/quicktime|max:51200', // Max 50MB
        ]);

        $pathOrLink = $miceKit->path_or_link;
        $originalFilename = $miceKit->original_filename;

        // Handle File Update
        if ($request->type === 'file' && $request->hasFile('file')) {
            // Delete old file if it exists
            if ($miceKit->path_or_link && Storage::disk('private')->exists($miceKit->path_or_link)) {
        ...
        elseif ($request->type === 'video' && $request->hasFile('video_file')) {
            ...
            $file = $request->file('video_file');
            $pathOrLink = $file->store('mice_kit_videos', 'private');
            $originalFilename = $file->getClientOriginalName();
        }
        ...
        return redirect()->route('admin.mice-kits.index')->with('success', 'MICE Kit item updated successfully.');
    }
